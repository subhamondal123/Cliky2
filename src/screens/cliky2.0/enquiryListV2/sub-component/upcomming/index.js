...
import { AssignedModal, BigTextButton, CheckBox, DropdownInputBox, EditAndDeleteModal, FilterModal, FloatingButton, ListViewModal, Loader, Modal, NoDataFound, TextInputBox } from "../../../../../shared";
...
class UpcommingList extends React.Component {
  ...
  _apiCallRes = async () => {
    ...
    let dataReq = {
      ...
      "enquirySourceId": this.state.enquirieSource,
      "enquiryTypeId": this.state.contactTypeId,
      "ownPhoneNoText": this.state.ownerPhoneNo,
      "recordType": this.state.reportType ? this.state.reportType : "1",
      "assignedId": this.state.subordinateId ? this.state.subordinateId : "",
      "status": this.state.status ? this.state.status : "1",
      "currentDateTime": DateConvert.formatYYYYMMDD(new Date())
  ...
  fetchListData = async (dataReq) => {
    let responseData = await MiddlewareCheck("sfaEnquiryList", dataReq, this.props);
    if (responseData) {
      if (responseData.status === ErrorCode.ERROR.ERROR_CODE.SUCCESS) {
        if (this.state.pageNum == 0) {
          ...
          if (enquiryData == null || enquiryData == undefined) {
            this.setState({
              enquiriesList: enquiryListData.enquiryList,
              totalDataCount: enquiryListData.totalCount
            })
            await StorageDataModification.sfaEnquiryListData(enquiryListData, "store");
          ...
          let enquiryListData = enquiryModifyData(responseData);
          if (enquiryListData.enquiryList == 0) {
            this.state.isApiCall = false;
          }
          this.setState(this.state);
          this.setState({
        ...
        if (this.state.pageNum == 0) {
          await StorageDataModification.sfaEnquiryListData({}, "clear");
          this.setState({
            pageNum: 0,
            limit: 10,
            totalDataCount: 0,
          ...
          <View style={[styles.blueBox]}>
            <View style={styles.blueViewFlex}>
              ...
              <TouchableOpacity style={{ marginLeft: "5%", flex: 1 }} activeOpacity={0.9} onPress={() => this.onShowHideData(item)}>
                ...
                <Text numberOfLines={1} style={styles.textDealer}>Source : {item.enquerySource}</Text>
              </TouchableOpacity>
              {item.showHide ?
                item.visitFor == "self" ?
                  <TouchableOpacity style={styles.addVisitsButton}
                    activeOpacity={0.9}
                    onPress={() => this.onAddVisit(item)} disabled={!this.state.initialApiCall}>
          ...
          {item.showHide ? <React.Fragment>
            <View>
              <View style={styles.textFlexView}>
                ...
                <View style={{ flex: 1 }}>
                  <Text style={styles.headerText}>Customer Type</Text>
                  <Text style={styles.textVisites}>{item.enquerySourceName}</Text>
                </View>
                <View style={styles.iconView}>
                ...
                <View style={{ flex: 1 }}>
                  ...
                  <Text style={styles.textVisites}>{DateConvert.viewDateFormat(item.assignDatetime)}</Text>
                </View>
              </View>
              <View style={styles.textFlexView}>
                <View style={styles.iconView}>
                ...
                <View style={{ flex: 1 }}>
                  ...
                  <Text key={key} style={styles.textVisites} >{item.businessAddress}</Text>
                </View>
                <View style={styles.iconView}>
                  <Image source={ImageName.CALL_ICON} style={styles.iconImg} />
                </View>
                <TouchableOpacity style={{ flex: 1 }} activeOpacity={0.9} onPress={() => this.linkPhone(item.ownerPhoneNo)} disabled={item.ownerPhoneNo == undefined || item.ownerPhoneNo == null ? true : false}>
                  <Text style={styles.headerText}>Owner Phone</Text>
              ...
              <View style={styles.textFlexView}>
                ...
                <View style={{ flex: 1 }}>
                  <Text style={styles.headerText}>Assigned By</Text>
                  <Text key={key} style={styles.textVisites} >{item.assignedByUserName}</Text>
                </View>
                <View style={styles.iconView}>
                  <Image source={ImageName.CALENDER_LOGO} style={styles.iconImg} />
                </View>
                <View style={{ flex: 1 }} >
                  <Text style={styles.headerAssignedText}>Assigned Date & Time</Text>
                  <Text key={key} style={styles.textAssignedVisites}>{item.assignDate}</Text>
                </View>
              </View>
              <View style={[styles.textFlexView, { borderBottomWidth: 0 }]}>
                ...
                <View style={{ flex: 1 }}>
                  ...
                  <Text key={key} style={styles.textVisites} >{item.assignToUserName}</Text>
                </View>
                {item.leadFeedback.length > 0 ?
                  <View style={{ flexDirection: 'row' }}>
                    <TouchableOpacity style={styles.conversionButton} activeOpacity={0.9} disabled={!this.state.initialApiCall} onPress={() => this.onFeedbacks(item)}>
                      <View style={styles.subCircel}>
                        <Image source={ImageName.CONVERSION} style={styles.subImgLogo} />
                      </View>
                      <Text style={styles.subBoxText}>Feedbacks</Text>
                    </TouchableOpacity>
                  </View>
                  :
                  null
  ...
  _deleteItem = () => {
    ...
    for (let i = 0; i < allItems.length; i++) {
      if (this.state.selectedContactItem.enqueryId == this.state.enquiriesList[i].enqueryId) {
        allItems.splice(i, 1);
      }
    }
    this.state.enquiriesList = allItems;
  ...
  _onFilterWithApi = async (data) => {
    this.state.contactTypeId = data.selectedContactTypeObj.id ? data.selectedContactTypeObj.id.toString() : "";
    this.state.enquirieSource = data.selectedEnquiriesObj.id ? data.selectedEnquiriesObj.id.toString() : "";
    this.state.ownerPhoneNo = data.phone ? data.phone : "";
    ...
    this.state.status = data.selectedStatus.id ? data.selectedStatus.id : "1";

    this.setState({
      contactTypeId: this.state.contactTypeId,
      enquirieSource: this.state.enquirieSource,
