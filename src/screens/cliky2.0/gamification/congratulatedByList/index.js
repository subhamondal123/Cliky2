...
class GamificationCongratulatedByList extends React.Component {
    constructor(props) {
        super(props);
        this.state = {
            toolTip: false,
    ...
    storeInitialData = async () => {
        let topGainData = await StorageDataModification.gamificationCongratulatedByList({}, "get");
    ...
    fetchListData = async (dataReq) => {
        let responseData = await MiddlewareCheck("likeListOfUser", dataReq, this.props);
            ...
            if (responseData.status === ErrorCode.ERROR.ERROR_CODE.SUCCESS) {
                if (this.state.pageNum == 0) {
                    ...
                    if (topGainData == null || topGainData == undefined) {
                        ...
                        await StorageDataModification.gamificationCongratulatedByList(topGainListData, "store");
                ...
                if (this.state.pageNum == 0) {
                    await StorageDataModification.gamificationCongratulatedByList({}, "get");
                    this.setState({
                        pageNum: 0,
                        limit: 10,
                        totalDataCount: 0,
    ...
    changeConversionStatus = async (value) => {
        ...
        //...............
        let pjpData = await StorageDataModification.gamificationCongratulatedByList({}, "get");
        ...
        await StorageDataModification.gamificationCongratulatedByList(pjpData, "store");
    }
    ...
    ListData = (item, key) => {
        const onPressThanks = async (item, index) => {
            if (item.thanksBack == 0) {
                item.thanksBack = 1
            } else {
                item.thanksBack = 0
            }
            this.setState({ allList: this.state.allList })
            let dataReq = {
                "thanksToId": item.userId,
                "createdAt": DateConvert.fullDateFormat(new Date()),
                "isThanks": item.thanksBack.toString()
            }
            let responseData = await MiddlewareCheck("thanksToUser", dataReq, this.props);
            if (responseData === false) {
            } else {
                if (responseData.status === ErrorCode.ERROR.ERROR_CODE.SUCCESS) {
                    Toaster.ShortCenterToaster(responseData.message)
            ...
            <View>
                <View style={styles.flexRow}>
                    <View style={[styles.circelBackground]}>
                        <Image source={{ uri: App_uri.CRM_BASE_URI + item.profileImgUrl }} style={styles.circelImg} />
                    </View>
                    <View style={styles.listMainView}>
                        <Text style={styles.listheaderText}>{item.userName}</Text>
                        <Text style={styles.listDateText}>{item.createdAt}</Text>
                    </View>
                    {item.thanksBack == 0 ?
                        <TouchableOpacity style={styles.listView} onPress={() => onPressThanks(item, key)}>
                            <View style={styles.flewRowView}>
                                <Text style={styles.textCoinNumber}>Thanks</Text>
                            </View>
                        </TouchableOpacity>
                        :
                        <View style={styles.activeListView} >
                            <View style={styles.flewRowView}>
                                <Text style={styles.activeTextCoinNumber}>Thanks</Text>
                            </View>
                        </View>
                    }
            ...
            <SafeAreaView style={styles.container}>
                ...
                {this.state.pageLoader ?
                    <View style={{marginTop:30, marginHorizontal:10}}>
                        <SkeletonPlaceholder>
                            <View style={{height:40,borderRadius:10,marginBottom:5}}/>
                    ...
                    <React.Fragment>
                        <View style={styles.mainView}>
                            ...
                            <Text style={styles.textTopGain}>Congratulated By</Text>
...
export default connect(mapStateToProps, mapDispatchToProps)(GamificationCongratulatedByList);
