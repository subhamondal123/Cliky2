...
import { convertListData, leadModifyData, modifyPriorityStatus, modListData, modListData_onApprove } from "./Function";
...
class LeadList extends React.Component {
  constructor(props) {
    ...
    this.state = {
      ...
      isRevertModal: false,
      approveStatus: approvalStausData,
      selectedApproveStatusDataObj: {},
      approveRemark: "",
      revertRemark: "",
      revertRemarkActive: false
    };
  }
  ...
  fetchListData = async (dataReq) => {
    let responseData = await MiddlewareCheck("fetchLeadList", dataReq, this.props);
    if (responseData) {
            ...
            <View style={styles.blueViewFlex}>
              ...
              <TouchableOpacity style={{ marginLeft: "5%", flex: 1 }} activeOpacity={1} onPress={() => this.onShowHideData(item)}>
                <View style={{ flexDirection: "row" }}>
                  <Text style={[styles.saiEnterprisesText, { flex: 1 }]} >{item.leadName}</Text>
                </View>
              ...
              <View>
                <View style={styles.textFlexView}>
                  ...
                  <View style={{ flex: 1 }}>
                    ...
                    <Text style={styles.textVisites}>{item.LeadOwner}</Text>
                  </View>
                  <View style={styles.iconView}>
                ...
                <View style={[styles.textFlexView]}>
                  ...
                  <View style={{ flex: 1 }}>
                    <Text style={styles.headerText}>Related Account</Text>
                    {/* {(item.email.split(",")).map((obj, key) => ( */}
                    <Text key={key} style={styles.textVisites}>{item.organizationName}</Text>
                    {/* ))} */}
                  </View>
                  <View style={styles.iconView}>
                    <Image source={ImageName.CALL_ICON} style={styles.iconImg} />
                  ...
                  <View style={{ flex: 1 }}>
                    <Text style={styles.headerText}>Related Contact</Text>
                    <Text style={styles.textVisites} >{item.contactFullName}</Text>
                  </View>
                </View>
                <View style={[styles.textFlexView]}>
                  <View style={styles.iconView}>
                    <Image source={ImageName.CALENDAR_VISIT} style={styles.iconImg} />
                  </View>
                  <View style={{ flex: 1 }}>
                    <Text style={styles.headerText}>Expected Closed Date</Text>
                    <Text style={styles.textVisites}>{item.expClosingDate}</Text>
                  </View>
                  <View style={styles.iconView}>
                    <Image source={ImageName.CALL_ICON} style={styles.iconImg} />
                  </View>
                ...
                <View style={[styles.textFlexView, { borderBottomWidth: 0 }]}>
                  ...
                  <View style={{ flex: 1 }}>
                    <Text style={styles.headerText}>Lead Stage</Text>
                  ...
                  <View style={{ flex: 1 }}>
                    <Text style={styles.headerText}>Lead Value</Text>
                    <Text style={styles.textVisites} >{item.leadValue}</Text>
                  </View>
                </View>
              </View>
            </React.Fragment>
  ...
  _onPressToolTip = (item) => {
    ...
    return (
      <Tooltip
        ...
        content={
          <ScrollView>
            {item.approvedStatus == "0" && item.enqueryRefId != "0" ?
              <TouchableOpacity
                style={styles.tooltipListView}
                onPress={() => onContactTooltipClick("revert", item)}
                activeOpacity={0.7}
              >
                <Text style={styles.tooltipText}>Revert</Text>
              </TouchableOpacity>
              :
              null
            }
  ...
  _onRevertModal = (item) => {
    ...
    if (this.state.isRevertModal) {
      this.state.isRevertModal = false;
      this.setState({
        isRevertModal: this.state.isRevertModal,
        selectedContactItem: item
      })
    } else {
      this.state.isRevertModal = true;
      this.state.enquiryList = convertListData(this.state.enquiryList, item);
      this.setState({
        isRevertModal: this.state.isRevertModal,
        selectedContactItem: item
  ...
  _onSelectApproveStatus = (value) => {
    this.setState({ selectedApproveStatusDataObj: value })
  }

  _onChangeApproveRemarks = (value) => {
    this.setState({ approveRemark: value })
  }

  _onCloseApproveModal = () => {
    this.setState({ isApproveModal: false, selectedApproveStatusDataObj: {}, approveRemark: "" })
  }

  _onApprove = async () => {
    if (this.state.selectedApproveStatusDataObj.id == undefined || this.state.selectedApproveStatusDataObj.id == null) {
      Toaster.ShortCenterToaster("Please select Approval Status !")
    } else if (this.state.approveRemark == undefined || this.state.approveRemark == null) {
      ...
      let reqData = {
        "refIdArr": [
          {
            "refId": this.state.selectedContactItem.leadId
          },
        ],
        "refType": "3",
        "approvedRemark": this.state.approveRemark,
        "approvedStatus": this.state.selectedApproveStatusDataObj.id,
      }
      let responseData = await MiddlewareCheck("generalApprove", reqData, this.props);
      if (responseData) {
        if (responseData.status === ErrorCode.ERROR.ERROR_CODE.SUCCESS) {
          Toaster.ShortCenterToaster(responseData.message);
          this.state.leadList = await modListData_onApprove(this.state.leadList, this.state.selectedContactItem, this.state.selectedApproveStatusDataObj.id);
          this.setState({ isApproveModal: false, selectedApproveStatusDataObj: {}, approveRemark: "" });
  ...
  _onChangeRevertRemark = (data) => {
    this.setState({ revertRemark: data })
  }

  _onCloseRevertMoval = () => {
    this.setState({ revertRemark: "", isRevertModal: false })
  }

  _onRevert = async () => {
    let reqData = {
      "leadId": this.state.selectedContactItem.leadId,
      "enqueryId": this.state.selectedContactItem.enqueryRefId,
      "revertRemarks": this.state.revertRemark
    }

    if (reqData.revertRemarks.length == 0) {
      Toaster.ShortCenterToaster("Please enter Remarks !")
    } else {
      let responseData = await MiddlewareCheck("revertLead", reqData, this.props);
      if (responseData) {
        if (responseData.status === ErrorCode.ERROR.ERROR_CODE.SUCCESS) {
          Toaster.ShortCenterToaster(responseData.message);
          for (let i = 0; i < this.state.leadList.length; i++) {
          ...
          this.setState({ leadList: this.state.leadList, isRevertModal: false, revertRemark: "", totalDataCount: this.state.totalDataCount - 1 });
        }
      }
    }
  }
    ...
    const modalSection = () => {
      return (
        <View>
          {/* revret modal */}
          <Modal
            isVisible={this.state.isRevertModal}
            onRequestClose={() => this._onCloseRevertMoval()}
            onBackdropPress={() => this._onCloseRevertMoval()}
            onBackButtonPress={() => this._onCloseRevertMoval()}
            children={
              <View style={styles.modalstatusview}>
                <View style={styles.modalStatusHeaderSec}>
                  <View style={styles.marginView}>
                    <Text style={styles.profileNameText}>Revert Lead</Text>
                  </View>
                </View>
                ...
                <View style={{ marginHorizontal: 20, marginTop: 20 }}>
                  <TextInputBox
                    borderRadius={20}
                    value={this.state.revertRemark}
                    onChangeText={(value) => this._onChangeRevertRemark(value)}
                    placeholder={"Remarks*"}
                    keyboardType={"default"}
                    isActive={this.state.revertRemarkActive}
                    onFocus={() => { this.setState({ revertRemarkActive: true }) }}
                    onBlur={() => { this.setState({ revertRemarkActive: false }) }}
                    height={90}
                ...
                <View style={{ marginTop: 20, flexDirection: 'row' }}>
                  <View style={{ flex: 1, marginHorizontal: '5%' }}>
                    <BigTextButton
                      ...
                      backgroundColor={Color.COLOR.BLUE.EBONY_CLAY}
                      text={"Cancel"}
                      onPress={() => this._onCloseRevertMoval()}
                    />
                  </View>
                  <View style={{ flex: 1, marginHorizontal: '5%' }}>
                    <BigTextButton
          ...
          <Modal
            ...
            onRequestClose={() => this._onApproveModal()}
            onBackdropPress={() => this._onApproveModal()}
            onBackButtonPress={() => this._onApproveModal()}
            children={
              <View style={styles.modalstatusview}>
                <View style={styles.modalStatusHeaderSec}>
                  <View style={styles.marginView}>
                    <Text style={styles.profileNameText}>Approve Lead</Text>
                  </View>
                </View>
                ...
                <View style={{ marginTop: 20 }}>
                  <View style={{ marginHorizontal: "5%" }}>
                    <DropdownInputBox
                      selectedValue={this.state.selectedApproveStatusDataObj.id ? this.state.selectedApproveStatusDataObj.id.toString() : "0"}
                      data={this.state.approveStatus}
                      onSelect={(value) => this._onSelectApproveStatus(value)}
                      headerText={"Approval Status*"}
                      selectedText={this.state.selectedApproveStatusDataObj.name ? this.state.selectedApproveStatusDataObj.name : "Select Status"}
                      selectedTextColor={this.state.selectedApproveStatusDataObj.name ? Color.COLOR.BLUE.LOTUS_BLUE : Color.COLOR.GRAY.SILVER}
                      isBackButtonPressRequired={true}
                      isBackdropPressRequired={true}
                    />
                    <View style={{ height: 10 }} />
                    <TextInputBox
                      value={this.state.approveRemark}
                      placeholder={"Enter Remark"}
                      onChangeText={(value) => this._onChangeApproveRemarks(value)}
                      keyboardType={"default"}
                      alignItems={"flex-start"}
                    ...
                    <View style={{ marginTop: 20, flexDirection: 'row' }}>
                      <View style={{ flex: 1, marginHorizontal: '5%' }}>
                        <BigTextButton
                          ...
                          backgroundColor={Color.COLOR.BLUE.EBONY_CLAY}
                          text={"Cancel"}
                          onPress={() => this._onCloseApproveModal()}
                        />
                      </View>
                      <View style={{ flex: 1, marginHorizontal: '5%' }}>
                        <BigTextButton
