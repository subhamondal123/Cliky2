...
class ContactListPage extends React.Component {
  ...
  storeInitialData = async () => {
    ...
    let contactData = await StorageDataModification.contactListData({}, "get");
    if (contactData == null || contactData == undefined) {
      this.setState({ pageLoader: true })
    } else {
      this.setState({
        contactList: contactData.contactList,
        totalDataCount: contactData.totalCount,
        pageLoader: false
      })
    }
  }
      ...
      if (responseData.status === ErrorCode.ERROR.ERROR_CODE.SUCCESS) {
        if (this.state.pageNum == 0) {
          ...
          if (contactData == null || contactData == undefined) {
            ...
            await StorageDataModification.contactListData(contactListData, "store");
          } else if (JSON.stringify(contactData.contactList) === JSON.stringify(contactListData.contactList)) {
            this.setState({
              contactList: contactListData.contactList,
              totalDataCount: contactListData.totalCount
            });
            if (contactData.totalCount !== contactListData.totalCount) {
              await StorageDataModification.contactListData(contactListData, "store");
            }
          } else {
            this.setState({
        ...
        if (this.state.pageNum == 0) {
          await StorageDataModification.contactListData({}, "clear");
          this.setState({
            pageNum: 0,
            limit: 10,
            totalDataCount: 0,
