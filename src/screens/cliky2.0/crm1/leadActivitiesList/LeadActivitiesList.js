...
import { ImageName, Color, FontSize, FontFamily } from "../../../../enums";
...
class leadActivityList extends React.Component {
    constructor(props) {
        super(props);
        this.state = {
            leadactivitiesList: [],
    ...
    _onLoad = async () => {
        ...
        let pastActivityResponseData = await MiddlewareCheck("getLeadsActivityLog", reqPastActivityData, this.props);
        if (pastActivityResponseData === false) {
            this._onNetworkError();
        } else {
            if (pastActivityResponseData.status === ErrorCode.ERROR.ERROR_CODE.SUCCESS) {
                if (pastActivityResponseData.response.activityListData.length < 10) {
                    this.setState({
                        loadMore: false
                    })
                }
                this.state.leadactivitiesList = [...this.state.leadactivitiesList, ...pastActivityResponseData.response.activityListData];
                this.setState({
        ...
        //         type: "up"
        //     }
        //     let upcomingActivityResponseData = await MiddlewareCheck("getLeadsActivityLog", reqUpcomingActivityData);
    ...
    listView = (item, key) => {
        return (
            <View style={styles.listDataView} key={key}>
                <Text style={[styles.textList, { flex: 0.25 }]}>{item.item.activityTypeName}</Text>
                <Text style={[styles.textDate, { textAlign: 'center', flex: 0.3 }]}>{DateConvert.formatYYYYMMDD(item.item.dueDate)}</Text>
                <Text style={[styles.nameText, { flex: 0.25 }]}>{item.item.firstName + " " + item.item.lastName}</Text>
                <Text numberOfLines={4} style={[styles.nameText, { flex: 0.2 }]}>{item.item.remarks}</Text>
                {/* <TouchableOpacity
                    activeOpacity={0.9}>
            ...
            <SafeAreaView style={styles.container}>
                ...
                <View style={{ marginHorizontal: '2%', marginTop: 8 }}>
                    <View style={{ flexDirection: 'row' }}>
                        <TouchableOpacity style={{ marginTop: 2, flex: 0.2 }}
                            activeOpacity={0.7}
                        ...
                        <View style={{ flex: 0.6 }}>
                            <View style={{ flexDirection: 'column' }}>
                                <Text style={{ color: Color.COLOR.BLUE.VIOLET_BLUE, fontSize: FontSize.SM, fontFamily: FontFamily.FONTS.INTER.BOLD, textAlign: 'center' }}>Lead Activity List</Text>
                            </View>
                        </View>
                        <View style={{ flex: 0.2 }} />
                    </View>
...
export default connect(mapStateToProps, mapDispatchToProps)(leadActivityList);
