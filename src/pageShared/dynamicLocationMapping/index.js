...
class DynamicLocationMapping extends React.Component {
    constructor(props) {
        super(props);
        this.state = {
            mapData: [],
    ...
    _load = async () => {
        ...
        if (this.props.type == "lastHierarcyField") {
            ...
            if (responseData) {
                ...
                this.setState({ beatLoader: false })
            }
        } else {
    ...
    onSelectLastField = async (value, arr) => {
        this.state.selectedLastFieldArr = value.id;
        let filterData = [
            {
        ...
        //     filterData.push({
        //         "hierarchyDataId": value.value[i],
        //         "hierarchyTypeId": value.selectItem.hierarchyTypeId
        //     });
        // }
        this.props.onApiCallData({ value: filterData, name: value.name })
    ...
    getUserImmediateChildData = async (value, selectParentData, index) => {
        ...
        let responseData = await MiddlewareCheck("getUserImmediateChildData", { "hierarchyTypeId": selectParentData.hierarchyTypeId, "hierarchyDataIdArr": tempHierarchyDataArr, "hierarchyChildTypeId": this.state.mapData[(index + 1)].hierarchyTypeId });
                        ...
                        <>
                            ...
                            /> */}
                                {this.props.screenName == undefined || this.props.screenName == null ?
                                    <DropdownInputBox
                                        // additionalMainTouchStyle={item.errorCheckData ? { borderColor: Color.COLOR.RED.LIGHT_RED, borderWidth: 1 } : {}}
                                        selectedValue={this.state.selectedLastFieldArr ? this.state.selectedLastFieldArr.toString() : "0"}
                                        data={this.state.lastFieldArr}
                                        onSelect={(value) => this.onSelectLastField(value)}
                                        headerText={this.state.lastLevelLocationTypeName}
                                        isBackButtonPressRequired={true}
                                        isBackdropPressRequired={true}
                                        isSearchable={true}
                                    ...
                                    <DropdownInputBox
                                        ...
                                        isSearchable={true}
                                        borderRadius={10}
                                        additionalBoxStyle={{ borderColor: '#000', borderWidth: 0.4 }}
                                    />
                                }
                            </React.Fragment>
                        </>
                                                        ...
                                                        {this.props.screenName == undefined || this.props.screenName == null ?
                                                            ...
                                                            this.props.screenName == "Crm" ?
                                                                <DropdownInputBox
                                                                    ...
                                                                    isSearchable={true}
                                                                    borderRadius={10}
                                                                    additionalBoxStyle={{ borderColor: '#000', borderWidth: 0 }}
                                                                />
                                                                :
                                                                this.props.screenName == "Mms" ?
                                                                    <DropdownInputBox
                                                                        // additionalMainTouchStyle={item.errorCheckData ? { borderColor: Color.COLOR.RED.LIGHT_RED, borderWidth: 1 } : {}}
                                                                        selectedValue={item.selectedId ? item.selectedId.toString() : ""}
                                                                        data={item.fileItem}
...
export default DynamicLocationMapping;